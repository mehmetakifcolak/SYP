import { Decorators, deepClone, GridUtils, IGetEditValue, indexOf, IReadOnly, ISetEditValue, SaveRequest, ServiceOptions, ServiceResponse, stripDiacritics, ToolButton, trimToNull, EntityGrid, EntityDialog} from "@serenity-is/corelib"
...
export class GridEditorBaseWithOption<TEntity, TOptions> extends EntityGrid<TEntity, TOptions>
    ...
    private matchContains(item): boolean {
        ...
        for (let prop in item) {
            result = stripDiacritics(String(item[prop] || '')).toLowerCase().indexOf(this.searchText) >= 0;
